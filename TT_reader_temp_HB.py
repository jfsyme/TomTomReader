# -*- coding: utf-8 -*-
"""
Created on Wed Sep 23 09:47:20 2020
...
# retrieval code which we are skipping for now
'''
outlist=[]
faillist = []
...
#create list of countries based on first three letters of URL codes
ctrylist = []
for i in citylist:
    x = i[:3]
    if x in ctrylist:
...
#create dictionary with countries as keys but no values
citydict = {k:[] for k in ctrylist}

#populate dictionary with URLs as values
for i in citylist:
    citydict[i[:3]].append(i)


for i in ctrylist:
    for j in citydict[i]:
        #create df of congestion for these cities
        #create df of population for these cities
        #weighted average
        #output will be a df of pop wtd congestion
